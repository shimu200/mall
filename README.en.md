...
#### 软件架构
1.采用分布式框架，将原本应该是一个的数据库，按照功能模块分成了多个数据库，减轻了单个数据库的压力；
2.前后端分离开发，后台管理系统，通过renrenfast开源框架，快速开发，主要用于实现商品和库存模块的功能；
3.用户前端的页面，通过thymeleaf的模板，快速修改而成；
...
|   nodes.js    | 14.14.0|                https://nodejs.org/en/download/               |
...
```
# gulimall
192.168.0.4 gulimall.com
192.168.0.4 search.gulimall.com
192.168.0.4 item.gulimall.com
192.168.0.4 auth.gulimall.com
192.168.0.4 cart.gulimall.com
192.168.0.4 order.gulimall.com
192.168.0.4 member.gulimall.com
192.168.0.4 seckill.gulimall.com
以上端口换成自己Linux的ip地址
```
...
- 克隆前端项目 `renren-fast-vue` 以 `npm run dev` 方式去运行 前端运行前，请先安装node.js v14.14.0
